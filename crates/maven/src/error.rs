...
/// Errors from the toolchain-free Maven matcher: reading a `gradle.lockfile`/`pom.xml` or a
/// file under the offline OSV DB mirror.
///
/// Like the other feeders, a present-but-broken input fails **closed** — an unreadable
/// manifest, or a corrupt advisory record, is an honest gap, never a false-clean scan. The
...
#[non_exhaustive]
pub enum MavenError {
    /// A `gradle.lockfile`/`pom.xml`, or a file under the offline OSV DB at `path`, could not
    /// be read or parsed. The underlying error is the [`source`](std::error::Error::source).
    #[error("maven scan {path}: {source}")]
    Db {
...
impl MavenError {
    /// Build a [`MavenError::Db`] from a path and any [`DbError`] source.
