...
/// Why a `Package.resolved` or a file under the offline OSV DB could not be used (shared across
/// feeders).
pub use fleetreach_core::osv::DbError;

/// Errors from the toolchain-free Swift matcher: reading a `Package.resolved` or a file under
/// the offline OSV DB mirror.
///
...
#[non_exhaustive]
pub enum SwiftError {
    /// A `Package.resolved`, or a file under the offline OSV DB at `path`, could not be read
    /// or parsed. The underlying error is the [`source`](std::error::Error::source).
    #[error("swift scan {path}: {source}")]
    Db {
...
impl SwiftError {
    /// Build a [`SwiftError::Db`] from a path and any [`DbError`] source.
